...
class CodeDial():
    def __init__(self):
        ...
        self.widget.setStyleSheet(
            "border-radius: 10px;border: 2px solid {0}; background-color: {1}; color:{0};".format(FONT_COLOR_PRIMARY, BUTTON_BG_COLOR))
...
#    mfarcBtn=QPushButton("SET MFARC")
    ...
    newCodeBtn.setStyleSheet(
        "border-radius: 7px;border: 2px solid {0}; background-color: {1}; color:{0};".format(FONT_COLOR_PRIMARY, BUTTON_BG_COLOR))
    newCodeBtn.setFixedHeight(btnHei)
    newCodeBtn.clicked.connect(
        lambda: CODE.get_code_then_set(chooseSourceBtn, mfa))
...
# TODO make version for pinetime
